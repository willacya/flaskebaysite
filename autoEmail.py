...
def format_time(time):
    time -= timedelta(hours=-1) # daylight savings
...
def ebay_results(keyword, user):

    origin = user.location

    if user.deadline == None:
        deadline = 90
    else:
        deadline = int(user.deadline)*60
    ...
    try:
        ...
        listing = pd.DataFrame({'Keywords':keywords,
                                ...
                                'Link':links,
                                'Ending':endings})
        if user.driveTime != None:
            listing = listing[listing.DrivingTime <= float(user.driveTime)*60]
...
for user in users:
    ...
    for keyword in keywords:
        total += 1
        listings, df_length = ebay_results(keyword, user)
        no_items += df_length
        if len(listings) < 1:
    ...
    subject = str(no_items) + " Items Found on Ebay Expiring Soon!" if no_items >1 else str(no_items) + " Item Found on Ebay Expiring Soon!"
